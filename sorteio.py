# Código por Gustavo Albuquerque - 20/04/2021 / 06/2022
...
def sorteio_copa():
    escolha_feita = False
    if escolha_feita is not True:
        print('Você gostaria de simular a Copa do Mundo com grupos reais ou sorteados conforme as regras da vida real?')
        escolha = input(
                        """
                        1 - Grupos Reais
                        2 - Grupos Sorteados
        ...
        print('Por favor, tente novamente digitando 1 ou 2.')

    def grupos_nao_reais():
        print("Simulando...")
        lista10 = []
        erroc = 0
        errou = 0
        with open('PAÍSES E ELENCOS.txt', encoding='utf-8') as arquivo:
            dados = arquivo.read()
    ...
    if grupos_reais is False:
        ...
        with open('PAÍSES E ELENCOS.txt', encoding='utf-8') as arquivo:
            ...
            for selecao in dados.splitlines():  # as linhas são separadas aqui!
                ...
                lista_times.append(dict_sel)

            ordem = sorted(lista_times, key=lambda k: k['País'])
            # Vou montar os grupos via ordem alfabética!

            grupoA = [ordem[26], ordem[12], ordem[27], ordem[17]]
            grupoB = [ordem[18], ordem[19], ordem[14], ordem[23]]
            grupoC = [ordem[1], ordem[2], ordem[22], ordem[24]]
            grupoD = [ordem[15], ordem[3], ordem[11], ordem[30]]
            grupoE = [ordem[13], ordem[9], ordem[0], ordem[20]]
            grupoF = [ordem[5], ordem[7], ordem[21], ordem[10]]
            grupoG = [ordem[4], ordem[28], ordem[29], ordem[6]]
            grupoH = [ordem[25], ordem[16], ordem[31], ordem[8]]
